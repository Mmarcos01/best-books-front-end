...
class BestBooks extends React.Component {
  ...
  handleDeleteBook = async (id) => {
    try {
      let bookData = await axios.delete(`${SERVER_URL}/books${id}?email=${this.state.email}`);
      this.setState({
        books: bookData.data
      });
    } catch (err) {
    ...
    let results = this.state.books.map(item =>
      <Carousel.Item key={item._id}>
        <div>
          ...
          <DeleteBooksButton onClick={this.handleDeleteBook(item._id)} />
      </Carousel.Item>
    );

    return (
      <div className="carousel">
        {isAuthenticated && this.state.books.length ?
          <>
            <Carousel>{results}</Carousel> <BookFormModal handleAddBook={this.handleAddBook} displayFlip={this.displayFlip} display={this.state.display} /> <AddBookButton displayFlip={this.displayFlip} display={this.state.display} />
          </> : ''}
      </div>
    );
  }
